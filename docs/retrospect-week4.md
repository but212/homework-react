...
useAuth는 분명 auth가 꺼지지 않은 상태이며 새로고침이나 상태 변경이 일어날 시 UI에 auth정보가 있음에도 반영이 안되어
충돌이 일어나는데 이를 해결하기 위하여 retry-strategy를 사용하였습니다. 이는 임시 장애 등 일시적 실패 상황에서
반복적으로 작업을 시도할 수 있도록 전략을 활용할 수 있도록 합니다.
즉 retry-strategy는 네트워크 에러나 일시적 장애가 발생했을 때 프로필 조회, 초기세션 확인, 로그인을 자동으로 재시도함으로써 세션이 제대로 초기화될 수 있도록 합니다. 단, 인증 에러(잘못된 비밀번호 등)는 재시도하지 않고 즉시 실패 처리합니다.

## 느낀점

이번 주의 학습내용은 머리가 지끈거릴 정도였습니다. 예측가능한 환경이 아닌 서버환경과 연계 시켜야 하기 때문에
굉장히 어려웠습니다. 특히 sql문을 작성하고 데이터베이스를 연동하는 부분은 거의 AI의 도움이 절실했습니다.
그리고 bun 템플릿 짜면서 해놓은 재시도 전략에 대한 코드가 도움이 될 줄은 꿈에도 몰랐습니다.
